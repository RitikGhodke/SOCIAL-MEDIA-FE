...
const Chats = () => {
    ...
    useEffect(() => {
        ...
        // ✅ Har 5 second mein refresh — mobile pe socket reliable nahi
        const interval = setInterval(getConversations, 5000)
        return () => clearInterval(interval)
    }, [])

    // ✅ Socket se bhi update karo
    useEffect(() => {
        ...
        socket.on("conversation-updated", () => {
            getConversations() // ✅ Fresh data lo
        })
                        ...
                        {loading ? (
                            ...
                            conv.map((item, index) => (
                                <ConvCard
                                    ...
                                    data={item}
                                    myId={myData._id}
                                    onDelete={getConversations} // ✅ delete ke baad refresh
                                />
                            ))
                        ) : (
                            <div className="flex flex-col items-center justify-center py-16 text-gray-400">
